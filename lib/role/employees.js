...
function editEmployees(id,companyId,userId,rank,managerId,state,newOwnerId,callback) {
    const employee = {
        id,
        companyId,
        userId,
    ...
    httpRequest.sendJsonRequest(url, headers, employee, method, (error, body) => {
        if (error) {
            ...
            if(body.data.ok > 0 && state == 5){
                house.transferExEmployeeCompanyHouse(userId,conpanyId,newOwnerId,callback)
            }
