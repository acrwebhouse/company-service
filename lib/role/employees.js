...
function getTeamMembersListByUserDataList(managerId,userDataList){
    ...
    for(let i = 0 ;i<userDataList.length;i++){
        if(managerId === userDataList[i].userId){
            result.push(userDataList[i])
    ...
    for(let i = 0 ;i<compareManagerIds.length; i++){
        ...
        for(let j = 0;j<userDataList.length;j++){
            if(compareManagerIds[i] === userDataList[j].managerId){
                nextCompareManagerIds.push(userDataList[j].userId)
                result.push(userDataList[j])
            }
        }
