...
function setCompanyIdToUser(companyId,userId,callback){
    getPersonalInfo(userId,(result,data)=>{
        if(result == true){
            ...
            user.companyId = companyId
            editUser(user._id,user.account,user.password,user.name,user.gender,user.roles,user.rolesInfo,user.houseIds,user.phone,user.mail,user.address,user.bornDate,user.companyId,callback)
        }else{
