...
function transferExEmployeeCompanyHouse(userId,companyId,newOwnerId,callback) {
    ...
    httpRequest.sendJsonRequest(url, headers, employee, method, (error, body) => {
        if (error) {
            ...
            if (body.data.ok > 0){
                reserveHouse.transferReserveHouse(userId,newOwnerId,companyId,()=>{})
...
function getHousesByBody(isDelete,skip,limit,minPrice,maxPrice,minPing,maxPing,floor,floor2,minRoom,maxRoom,minFloor,maxFloor,buildingType,city,area,parking,pet,manager,garbage,smoke,cook,textQuery,ownerArray,creatorArray,typeOfRental,belongType,belongId,sort,minCreateTime,maxCreateTime,callback){
    const house = {}
    ...
    if(utilsValue.isValid(creatorArray)){
        house.creatorArray = creatorArray;
    }
...
function getTeamUploadHouseCountsResult(teamHouseList,priceRange,teamMembers){
    for(let i = 0 ;i<teamMembers.length; i++){
        teamMembers[i].totalUploadCount = 0
        teamMembers[i].uploadCounts = []
        for(let j = 0 ;j<priceRange.length;j++){
            teamMembers[i].uploadCounts.push(0)
        }
    }
        ...
        for(let j = 0 ;j<priceRange.length; j++){
            if(teamHouseList[i].price>=priceRange[j].min && teamHouseList[i].price<=priceRange[j].max){
                for(let k = 0 ;k<teamMembers.length;k++){
                    if(teamHouseList[i].creator === teamMembers[k].userId){
                        teamMembers[k].uploadCounts[j]++
                    }
                }
            }
        }
    ...
    for(let i = 0 ;i<teamMembers.length; i++){
        for(let j = 0 ;j<teamMembers[i].uploadCounts.length;j++){
            teamMembers[i].totalUploadCount += teamMembers[i].uploadCounts[j]
        }
    }
    return teamMembers;
}
    ...
    employees.getTeamMembers(managerId,companyId,(result,data)=> {
        if(result === true){
            ...
            for(let i = 0 ;i < data.length;i++){
                creatorArray.push(data[i].userId)
            }
            getHousesByBody(false,0,9999999,'','','','','','','','','','','','','','','','','','','','',[],creatorArray,'',2,companyId,'',minCreateTime,maxCreateTime,(result,data)=>{
                if(result === true){
                    callback(true,getTeamUploadHouseCountsResult(data,priceRange,teamMembers))
                }else{
                    callback(false,data)
                }
            })
...
function getHouse(id,callback) {
    let url = config['house-basic-server'].location+'/'+config['house-basic-server'].restApi.getHouse+'?id='+id+'&isDelete=false' ;
    const method = 'GET';
    const headers = {};
    httpRequest.sendGetRequest(url, headers, method, (error, body) => {
        if (error) {
            console.log('===getHouse==url=')
            console.log(url)
            console.log('===getHouse==error=')
            console.log(error)
            console.log('===getHouse==body=')
            console.log(body)
            callback(false,body);
        } else {
            try{
...
function editHouse(house,callback){
    ...
    httpRequest.sendJsonRequest(url, headers, house, method, (error, body) => {
        if (error) {
            ...
            else if(body.data.nModified && body.data.nModified > 0){
